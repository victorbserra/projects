...
class WebScraper:
    ...
    def header_configuration(self, headless=True):
        ...
        if self.proxy_ip_port and self.proxy_user_pass:
            proxy = Proxy()
            proxy.proxy_type = ProxyType.MANUAL
            proxy.http_proxy = f'{self.proxy_user_pass}@{self.proxy_ip_port}'
            proxy.ssl_proxy = f'{self.proxy_user_pass}@{self.proxy_ip_port}'
            capabilities = webdriver.DesiredCapabilities.CHROME
            proxy.add_to_capabilities(capabilities)
            self.driver = webdriver.Chrome(service=webdriver_service, options=chrome_options, desired_capabilities=capabilities)
        else:
